                    ...
                    names.map( ( name, index ) => {
                        setTimeout( ()=>{
                            filterDataFromFirebase( $( this ).attr( 'data-sell' ).toLowerCase( ), 'category', name, snap => {
                                categories = `<a href="" data-ref="`+$( this ).attr( 'data-sell' ).toLowerCase( )+`" data-value="`+ name +`" data-order="category" class="black-text get-items"><div class="col s12 m3 animated zoomIn">
                                    <div class="loader card-panel row center `+ colorShade( ) +`">
                                        <span class="item-name">`+ name +`</span><br>
                                            <span class="item-count">`+ snap.numChildren( ) +`</span><span class="item-unit">items</span>
                                        </div>
                                    </div></a>`;
                                    if ( index === 0 ) {
                ...
                loadDataFromFirebase( navigate.ref, '', snapshot => {
                    ...
                    snapshot.forEach( snap => {
                        if ( snap.val( ).category === navigate.value ) {
                            ...
                            let item = `<a href="" class="black-text items"><div class="col s12 m3 animated zoomIn">
                            ...
                            loadDataFromFirebase( navigate.oldref, '', snap => {
                                ...
                                $( document ).on( 'click', '#save-sale', function ( e ) {
                                    ...
                                    if (
                                        ...
                                        let invoice     = prepareReceipt( id, sales.type, 'Sign', ( ( sales.cost - sales.delivery ).toFixed( 2 ) ), sales.delivery, sales.cost, sales.paid, sales.change );
